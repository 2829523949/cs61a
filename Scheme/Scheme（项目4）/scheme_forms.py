...
def do_and_form(expressions, env):
    ...
    """
    # BEGIN PROBLEM 12
    current_place=expressions
    while not current_place.rest is nil:
        current_value=scheme_eval(current_place.first)
        if is_scheme_false(current_value):
            return False
        else:
            current_place=current_place.rest
    current_value=scheme_eval(current_place.first)
    if is_scheme_false(current_value):
        return False
    else:
...
def do_or_form(expressions, env):
    ...
    current_place=expressions
    while not current_place.rest is nil:
        if is_scheme_true(scheme_eval(current_place.first)):
            return scheme_eval(current_place.first)
        else:
            current_place=current_place.rest
    return scheme_eval(current_place.first)
    # END PROBLEM 12
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            if expressions.rest is nil:
                ...
                return eval_all(expressions.rest)
            # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    box=[]
    current_place=bindings
    while not current_place is nil:
        box.append(current_place.first)
        current_place=current_place.rest
    reverse_box=reversed(box)
    for binding in reverse_box:
        validate_form(binding,2,2)
        names=Pair(binding.first,names)
        vals=Pair(scheme_eval(binding.rest),vals)
