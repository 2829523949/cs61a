...
def floor_div(args):
    ...
    assert args is Pair 
    current_place=args
    current_value=args.first
    while not args.rest is nil:
        current_value=current_value//(current_place.rest.first)
...
def eval_and(expressions):
    ...
    True
    """
    current_place_left=expressions.rest.first
    current_place_right=expressions.rest.rest
    def category(m,n):
        if (m is False) or (n is False):
            return False
        elif (m is nil) and (n is nil):
            return True 
        elif m is nil:
            return calc_eval(n)
        elif n is nil:
            ...
            return calc_eval(m)+calc_eval(n)
    def abstraction(x):
        while x is Pair:
            if x is False:
                return False 
                ...
                category(x.first,x.rest)
    result_left=abstraction(current_place_left)
    result_right=abstraction(current_place_right)
    return category(result_left,result_right)
bindings = {}
